...
- Always get another developer to review, not just a delivery lead. For the dashboard, that meant tagging both the DL and a developer. For the query work later, Joe would tag only [[Nick Hazekamp|Nick]], since a gnarly SQL query is not something [[Meg Kretz|Meg]] would enjoy wading through. Match the reviewer to the change.
